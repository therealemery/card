...
from tests.conftest import admin_headers, days_from_now, resolve_headers, set_expires_at
...
def test_resolve_suspended(api, project_with_card):
    proj, card = project_with_card
    api.patch(f"/api/cards/{card['card_key']}", json={"action": "suspend"}, headers=admin_headers(proj))
...
def test_resolve_revoked(api, project_with_card):
    proj, card = project_with_card
    api.patch(f"/api/cards/{card['card_key']}", json={"action": "revoke"}, headers=admin_headers(proj))
    body = _resolve(api, proj, card["card_key"]).json()
    assert body["valid"] is False
...
def test_resolve_requires_resolve_token(api, project_with_card):
    proj, card = project_with_card
    # 缺 header / 错误 token / admin_key 不能当 resolve_token 用
    ...
    assert api.post(
        "/api/resolve", json={"card_key": card["card_key"]},
        headers={"Authorization": f"Bearer {proj['admin_key']}"},
    ).status_code == 403
